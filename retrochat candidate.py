...
class ChatProvider(ABC):
    ...
    @abstractmethod
    async def _send_message_to_provider(self, message: str) -> str:
        pass

    async def send_message(self, message: str):
        self.add_to_history("user", message)
        response = await self._send_message_to_provider(message)
        self.add_to_history("assistant", response)
        self.save_history()
        if self.parameters.get("verbose", False):
            tokens = self.calculate_tokens(response)
            total_tokens = self.calculate_total_tokens()
            self.console.print(f"Response tokens: {tokens}", style="cyan")
            self.console.print(f"Total conversation tokens: {total_tokens}", style="cyan")
        return response
    ...
    def add_to_history(self, role: str, content: str):
        ...
        if self.parameters.get("verbose", False) and role == "user":
            ...
            total_tokens = self.calculate_total_tokens()
            self.console.print(f"Message tokens: {tokens}", style="cyan")
            self.console.print(f"Total conversation tokens: {total_tokens}", style="cyan")
    ...
    def display_history(self):
        if not self.chat_history:
            self.console.print("No previous chat history.", style="cyan")
        else:
            self.console.print("Chat history loaded from previous session:", style="cyan")
            for entry in self.chat_history:
                if entry.role == "user":
                    self.console.print(entry.content, style="green")
                else:
                    self.print_formatted_message(entry.content, style="yellow")
    ...
    def set_parameter(self, param: str, value: Any):
        if param in self.default_parameters or param in ["repeat_penalty", "frequency_penalty"]:
            ...
            if param != "verbose" or value:
                self.console.print(f"Parameter '{param}' set to {value}", style="cyan")
        else:
            self.console.print(f"Invalid parameter: {param}", style="bold red")

    def show_parameters(self):
        self.console.print("Current Parameters:", style="cyan")
        for param, default_value in self.default_parameters.items():
            current_value = self.parameters.get(param, default_value)
            if param == "max_tokens":
                self.console.print(f"max_tokens: {current_value} (sent as max_output_tokens to Gemini API)", style="green")
            else:
                self.console.print(f"{param}: {current_value}", style="green")
        
        if "frequency_penalty" not in self.default_parameters:
            self.console.print(f"frequency_penalty: {self.parameters.get('frequency_penalty', self.parameters.get('repeat_penalty', 1.1))}", style="green")
        
        if self.system_message:
            self.console.print(f"system: {self.system_message}", style="green")
    ...
    def format_markdown(self, text: str) -> List[Any]:
    ...
    def print_formatted_message(self, message: str, style: str = "yellow"):
        formatted_parts = self.format_markdown(message)
        for part in formatted_parts:
            if isinstance(part, Syntax):
                self.console.print(part)
            else:
                self.console.print(part, style=style)

    async def stream_markdown(self, message_generator):
        complete_message = ""
        code_block = ""
        in_code_block = False
        has_markdown = False
        ...
        def get_renderable():
            if not has_markdown:
                return Text(complete_message, style="yellow")
            if in_code_block:
                return Group(
                    Markdown(complete_message + "```" + code_block),
                    Markdown("```")
                )
            else:
                return Markdown(complete_message)

        with Live(get_renderable(), refresh_per_second=4, transient=False) as live:
            async for message_content in message_generator:
                    ...
                    if char == '`':
                        if in_code_block:
                            ...
                            if complete_message.endswith('``'):
                                in_code_block = True
                                complete_message = complete_message[:-2]
                            else:
...
class OllamaChatSession(ChatProvider):
    ...
    async def _send_message_to_provider(self, message: str) -> str:
        ...
        complete_message = await self.stream_markdown(message_generator())
        return self.format_message(complete_message)
...
class AnthropicChatSession(ChatProvider):
    ...
    async def _send_message_to_provider(self, message: str) -> str:
        ...
        # Don't add the message here, as it's already been added in the send_message method of ChatProvider

        async def message_generator() -> AsyncIterator[str]:
            try:
                ...
                self.console.print(f"Anthropic API Error: {str(e)}", style="bold red")
                yield ""
    ...
    def prepare_messages(self) -> List[Dict[str, str]]:
        ...
        # If there are no messages or the last message is from the assistant, add a dummy user message
    ...
    async def send_message(self, message: str):
        # Override the send_message method to ensure proper message handling
        self.add_to_history("user", message)
        response = await self._send_message_to_provider(message)
        self.add_to_history("assistant", response)
        self.save_history()
    ...
    def create_provider(provider_type: str, *args, **kwargs) -> ChatProvider:
        ...
        if provider_class:
            if provider_type == 'Anthropic':
                return provider_class(kwargs['api_key'], kwargs['model'], kwargs['history_manager'])
...
class ChatApp:
    ...
    async def select_anthropic_model(self) -> str:
        ...
        for idx, model in enumerate(models, 1):
            console.print(f"{idx}. {model}", style="green")
    ...
    async def switch_provider(self):
        ...
        if mode == '1':
            ...
            if not selected_model:
                return None
            new_session = self.provider_factory.create_provider('Anthropic', 
