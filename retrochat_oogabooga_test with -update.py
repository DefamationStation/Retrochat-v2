...
class CommandHandler:
    ...
    async def handle_command(self, command: str, session: ChatProvider):
        ...
        cmd, *args = cmd_parts + ['', '']
        if cmd == '/update':
            await self.handle_update_command(args[0])
        elif cmd == '/chat':
...
class ChatApp:
    ...
    "https://api.openai.com/v1/chat/completions", selected_model, self.history_manager)
        ...
        if new_session:
            ...
            self.save_last_provider_and_model(provider, character if provider == 'Oobabooga' else selected_model)
            return new_session
        return None
    ...
    async def start(self):
        try:
            ...
            try:
                ...
                console.print(f"Error during update check: {str(e)}", style="yellow")
                console.print("Continuing without checking for updates.", style="yellow")
            ...
            try:
                ...
                console.print(f"Error creating session: {str(e)}", style="yellow")
                self.current_session = None
            ...
            if not self.current_session:
                try:
                    ...
                    console.print(f"Error switching provider: {str(e)}", style="bold red")
                    console.print("Unable to initialize a chat session. Please check your network connection and API keys.", style="bold red")
                    return
            
            if not self.current_session:
                console.print("No valid chat session could be created. Exiting.", style="bold red")
                return
            ...
            while True:
                try:
                    ...
                    console.print("Operation cancelled. You can continue chatting or type '/exit' to quit.", style="yellow")
                except aiohttp.ClientError as e:
                    console.print(f"Network error occurred: {str(e)}", style="bold red")
            ...
            console.print("\nOperation interrupted by user. Exiting gracefully.", style="yellow")
        except Exception as e:
            console.print(f"A critical error occurred: {str(e)}", style="bold red")
            console.print("The application will now exit. Please report this issue if it persists.", style="bold red")
        finally:
            if self.current_session:
                self.current_session.save_history()
    ...
    async def check_for_updates(self):
        ...
        check_updates = EnvManager.get_env_variable("CHECK_UPDATES", "true").lower() == "true"

        if not check_updates:
            console.print("Update checks are disabled.", style="yellow")
            return False
        ...
        try:
            ...
            console.print("GitHub connection timed out. Skipping update check.", style="yellow")
            return False
        except Exception as e:
            console.print(f"Failed to check for updates: {str(e)}", style="bold red")
            return False
        ...
        latest_commit = await response.json()
        ...
        missed_commits = await self.get_missed_commits(repo_owner, repo_name, file_path, last_commit_hash)
        ...
        try:
            ...
            console.print(f"Failed to fetch the latest version: {str(e)}", style="bold red")
            return False
    ...
    async def get_missed_commits(self, repo_owner, repo_name, file_path, last_commit_hash):
        ...
        try:
            ...
            console.print("GitHub connection timed out. Unable to fetch missed commits.", style="yellow")
            return []
        except Exception as e:
            console.print(f"Failed to fetch commits: {str(e)}", style="bold red")
            return []

        if response.status != 200:
            console.print(f"Failed to fetch commits: {response.status}", style="bold red")
            return []
    ...
    async def handle_update_command(self, value: str):
        if value.lower() in ['true', 'false']:
            EnvManager.set_env_variable("CHECK_UPDATES", value.lower())
            status = "enabled" if value.lower() == 'true' else "disabled"
            console.print(f"Update checks are now {status}.", style="cyan")
        else:
            console.print("Invalid value. Use 'true' to enable or 'false' to disable update checks.", style="bold red")
...
async def main():
    if len(sys.argv) > 1 and sys.argv[1] == "--setup":
        ...
        try:
            ...
            console.print("The application has encountered a critical error and must exit.", style="bold red")
            sys.exit(1)
